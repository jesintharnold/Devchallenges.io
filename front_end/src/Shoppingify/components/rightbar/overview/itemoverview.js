...
export const Itemoverview=()=>{
  ...
  return (
    <>
    <div className="flex items-center text-shop-orange font-medium mb-5 cursor-pointer" onClick={()=>{dispatch({type:ISMOBILE,payload:false});dispatch_cart({type:OVERVIEW_STATE,payload:{
      status:false,
      categoryID:null,
      itemID:null
    }});
    ...
    <span className="block text-shop-orange my-3 text-base font-bold">name
    <p className="text-black font-bold text-xl mt-1 opacity-90">{data.items[0].item}</p>
    </span>
    
    <span className="block text-shop-orange my-3 text-base font-bold">category
    <p className="text-black font-semibold text-xl mt-1 opacity-90">{data.category}</p>
    </span>
    
    <span className="block text-shop-orange my-3 text-base font-bold">note
    <p className="break-words text-black  mt-2 leading-6 font-normal">{data.items[0].description}</p>
    </span>
    </div>

    <div className="px-2 py-6 bg-transparent w-full sticky bottom-0 bg-shop-right-back">
        <div className="flex items-center justify-evenly font-bold text-base">
            <button type='submit' className="px-2 py-1 rounded-lg capitalize bg-shop-orange text-white" onClick={()=>dispatch_cart({type:ADD_ITEM_LIST,
              payload:{
                categoryID:data._id,
