...
export const Preview=({data})=>{
    ...
    const copyLink=(e)=>{
        e.preventDefault();
        e.stopPropagation();
        navigator.clipboard.writeText(window.location.href+"image/"+data.id);
    ...
    <div className="flex flex-col text-center justify-between box-border">
        <span className="material-icons text-greentick text-3xl">check_circle</span>
        ...
        <div className={`w-full h-64 relative my-4 bg-transparent rounded-xl overflow-hidden flex justify-center items-center`}>
            ...
            <img onLoad={(e)=>{e.preventDefault(); setLoad(false);}} alt="Not found" src={data.url} className={`object-cover w-full h-full  ${load ? 'hidden' :'animate-accord'}`}/>
        </div>

        <div className='border-[1px] flex-nowrap flex box-border p-1 rounded-lg overflow-hidden items-center justify-between'>
            <p className='ml-1 mr-2 text-base  overflow-hidden whitespace-nowrap'>{window.location.href+"image/"+data.id}</p>
